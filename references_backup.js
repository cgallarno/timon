...
//$(function(){
  /*
    $.getScript = function (url) {
        ...
        if (url.indexOf('+') > -1){
          // configure plus.io module path
          script.src = 'App/Plugins/' + url.replace('+','') + '/module.js';
        }
        else {
          // default src path
          script.src = url;
        ...
        $("head").append(script);
        $script.bind("success", function(){alert('loaded')});
        $script.bind("error", function(){console.log("Error occurred while loading module: " + url)});
    };

    $.getScripts = function (urls) {
        if (urls.length==0)
          throw Error("scripts missing");
        ...
        $("head").append(script);
        $script.bind("load", function(){alert('loaded');});
        $script.bind("error", function(){console.log("Error occurred while loading module: " + url)});
    };

   // $.getScript('App/Plugins/jquery/module.js');
$.getScript('App/Plugins/cordova/module.js', function() {});
$.getScript('App/Plugins/movable-type/module.js', function() {});
$.getScript('App/Plugins/angular/module.js', function() {
  console.log('angular loaded');
  $.getScript('App/Plugins/angular-resources/module.js', function() {
      console.log('angular-resources loaded');
      $.getScript('App/Plugins/plus/module.js', function() {
          console.log('plus loaded');
        });
  });
});
$.getScript('App/Themes/'+active_theme+'/app.js', function() {});
...
/*
 $.getScript('App/Plugins/cordova/module.js', function() {
   ...
   for (var i = 0; i <= modules.length - 1; i++) {
      ...
      if (url.indexOf('+') > -1){
            // configure plus.io module path
            module_url = 'App/Plugins/' + url.replace('+','') + '/module.js';
      }
      else {
        // default src path
        module_url = url;
   ...
   /* $.getScript('+cordova');
    $.getScript('+movable-type');
    $.getScript('+angular');
    $.getScript('+angular-resources');    
    $.getScript('+plus');
    $.getScript('App/Themes/'+active_theme+'/app.js');    
    $.getScript('main.js');*/
 
   //angular.bootstrap(document, ['myApp']);
 //   console.log("bootstrapped should of worked by now.");
...
// function used to load Plus IO modules.
// Use the head.ready() at the end to specify functions to execute once they are all done loading.
// Do NOT use document.ready() since it may have already fired by the time the scripts start to load.
function LoadPlusIOModules(modules){for(var i=0;i<=modules.length-1;i++){var url=modules[i];var module_url;if(url.indexOf('+')>-1){module_url='App/Plugins/'+url.replace('+','')+'/module.js';}else{module_url=url;}head.js(module_url);}head.ready(function(){console.log('all modules loaded.');});}
var modules =['+jquery',
              '+cordova', 
              '+movable-type', 
              '+angular', 
...
LoadPlusIOModules(modules);

/*
head.js('App/Plugins/jquery/module.js',
        'App/Plugins/angular/module.js',
        'App/Plugins/angular-resources/module.js',
        'App/Plugins/cordova/module.js',
        'App/Plugins/movable-type/module.js',
        'App/Plugins/plus/module.js',
        'main.js',
        'App/Themes/'+active_theme+'/app.js', function() {
          console.log('all js modules loaded.');
...
*/
// if + is there, take folder name and lookup module
// else just take path of provided file 